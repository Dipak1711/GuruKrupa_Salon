        ...
        {gallery.map((item, idx) => (
          <div key={item.id || idx} className="glass-card" style={{ overflow: 'hidden' }}>
            <div style={{ position: 'relative', height: '220px' }}>
              ...
              <div style={{ position: 'absolute', top: '10px', left: '10px', backgroundColor: 'rgba(10, 12, 16, 0.85)', color: '#F3E5AB', padding: '3px 8px', borderRadius: '6px', fontSize: '0.75rem', fontWeight: 600 }}>
                {item.category}
              </div>
            </div>
            <div style={{ padding: '16px' }}>
              <h4 className="font-serif" style={{ fontSize: '1.15rem', color: '#F8FAFC', fontWeight: 600, marginBottom: '6px' }}>
                {item.title}
              </h4>
              <p style={{ fontSize: '0.84rem', color: '#CBD5E1', lineHeight: 1.4 }}>{item.description}</p>
              {item.stylist_name && (
                <div style={{ fontSize: '0.78rem', color: '#D4AF37', marginTop: '8px' }}>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} maxWidth="md" title="Add Gallery Showcase Item">
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <div>
              <label style={{ fontSize: '0.82rem', color: '#94A3B8', display: 'block', marginBottom: '6px' }}>Category *</label>
              <input type="text" className="salon-input" value={category} onChange={(e) => setCategory(e.target.value)} required />
            </div>
            <div>
              <label style={{ fontSize: '0.82rem', color: '#94A3B8', display: 'block', marginBottom: '6px' }}>Master Artist Name</label>
              <input type="text" className="salon-input" value={stylistName} onChange={(e) => setStylistName(e.target.value)} />
            </div>
          </div>
          <div>
            <label style={{ fontSize: '0.82rem', color: '#94A3B8', display: 'block', marginBottom: '6px' }}>Image URL *</label>
