  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
      ...
      <div className="glass-card" style={{ padding: '24px', overflowX: 'auto' }}>
        <h3 className="font-serif" style={{ fontSize: '1.35rem', color: '#F8FAFC', fontWeight: 600, marginBottom: '16px' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            ...
            <tbody>
              {myRecords.map((rec) => (
                <tr
                  key={rec.id}
                  style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.05)', fontSize: '0.88rem' }}
                >
                  <td style={{ padding: '14px' }}>
                    <div style={{ color: '#F8FAFC', fontWeight: 600 }}>#{rec.id.slice(-6).toUpperCase()}</div>
                    <div style={{ fontSize: '0.76rem', color: '#94A3B8' }}>{formatDateTime(rec.completed_at)}</div>
                  </td>

                  <td style={{ padding: '14px' }}>
                    <div style={{ color: '#F3E5AB', fontWeight: 500 }}>{rec.customer_name}</div>
                    <div style={{ fontSize: '0.76rem', color: '#64748B' }}>{rec.customer_phone}</div>
                  </td>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                      {rec.items.map((item) => (
                        <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.84rem' }}>
                          <span style={{ color: '#E2E8F0' }}>{item.service_name}</span>
                          <span style={{ color: '#94A3B8', marginLeft: '12px' }}>{formatPrice(item.unit_price)}</span>
                        </div>
                      ))}
                      {rec.discount > 0 && (
                  ...
                  <td style={{ padding: '14px' }}>
                    ...
                    >
                      {rec.payment.payment_method === 'UPI' && <QrCode size={13} color="#38BDF8" />}
                      {rec.payment.payment_method === 'Cash' && <Banknote size={13} color="#10B981" />}
                      {rec.payment.payment_method === 'Card' && <CreditCard size={13} color="#F59E0B" />}
